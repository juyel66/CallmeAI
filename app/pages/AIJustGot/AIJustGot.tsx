                ...
                return (
                  <>
                    {cards.map((card, index) => (
                      <div
                        key={index}
                        className={`absolute left-1/2 top-1/2 h-125 mt-10 md:w-80 w-74 -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-2xl transition-all duration-500 select-none ${
                          card.state.isActive ? "shadow-2xl shadow-purple-500/20" : "shadow-lg"
                        }`}
                        style={card.state.style}
                        onContextMenu={(e) => e.preventDefault()}
                      >
                        <div className="relative flex h-full w-full flex-col rounded-2xl bg-white p-3.5">
                          <Image
                            src={card.image}
                            alt={card.title}
                            className="mt-2 h-68 w-full  border-gray-200 rounded-2xl "
                            width={400}
                            height={560}
                          />
