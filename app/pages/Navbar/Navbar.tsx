...
const Navbar = () => {
    ...
    return (
        <header className="relative ">
            <nav className="mx-auto flex h-[76px] w-full max-w-[1240px] items-center justify-between px-4 sm:px-6 md:h-[92px] md:px-10">
                <Link
                    href="/"
                    onClick={closeMenu}
                ...
                >
                    <span className="relative block h-4 w-5">
                        <span
                            className={`absolute left-0 top-0 block h-[2px] w-5 bg-current transition-transform duration-300 ${
                        ...
                        />
                        <span
                            className={`absolute left-0 top-[14px] block h-[2px] w-5 bg-current transition-transform duration-300 ${
                                isMenuOpen ? "-translate-y-[7px] -rotate-45" : ""
                            }`}
                        />
                    </span>
            ...
            >
                <ul className="mx-auto flex w-full max-w-[1240px] flex-col gap-1 px-4 py-4 sm:px-6 md:px-10">
                    {navItems.map((item) => (
                        <li key={item.label}>
                            <Link
                                href={item.href}
                                onClick={closeMenu}
                                className={`block border-l-[4px] px-4 py-3 text-[18px] font-medium transition-colors duration-200 ${
                                    isActive(item.href)
